...
// Note: re-running preserves publishedAt (only set on create) but does
// bump updatedAt via Prisma's @updatedAt, which is correct -- an edit did
// happen.
...
If you want photos of this coastline that go beyond a phone snapshot from the sidewalk, for yourself, for a shoot, or aerial coverage of the whole bluff from a drone, [get in touch](mailto:law138@santacruzmtnphotography.com).`,
  },
  {
    ...
    content: `Natural Bridges is one of those places that photographs well almost by accident. Point a camera at the arch and you'll get something decent even on a bad day. But there's a real difference between a decent shot and the shot the place is actually capable of, and it comes down to knowing what to wait for.
